...
$mainScript = Join-Path $scriptPath $config.inputFile
...
$tempScriptFile = $null
if ($needEmbedCode) {
    Write-Host "`n准备将依赖文件的二进制内容嵌入到主脚本中..." -ForegroundColor Yellow
...
`$env:ZEROTIER_PORTABLE_ROOT = `$extractedPath
...
"@
    ...
    foreach ($pattern in $packageItems) {
        ...
        if ([string]::IsNullOrEmpty($dirPath)) {
            $fullPath = Join-Path $scriptPath $filePattern
        } else {
            $fullPath = Join-Path $scriptPath $dirPath
        ...
        # 解析通配符获取文件
        try {
            Write-Host "  正在搜索: $fullPath" -ForegroundColor Gray
            ...
            foreach ($file in $files) {
                ...
                Write-Host "  处理文件: $relativePath ($([Math]::Round($fileSize / 1KB, 2)) KB)" -ForegroundColor Gray
    ...
    # 显示嵌入文件统计
    Write-Host "已嵌入 $filesAdded 个文件 (总计找到 $totalFiles 个匹配), 嵌入数据大小约 $([Math]::Round($totalSize / 1MB, 2)) MB" -ForegroundColor Green

    if ($filesAdded -eq 0) {
        Write-Host "警告: 未嵌入任何文件，检查配置中的路径是否正确" -ForegroundColor Red
        $needEmbedCode = $false  # 不需要继续生成临时文件
    } else {
        # 将嵌入代码和原始内容合并
        $newContent = $embeddedCode + $originalContent

        # 写入临时脚本文件 (使用UTF-8无BOM编码)
        try {
            $utf8NoBomEncoding = New-Object System.Text.UTF8Encoding $false
            [System.IO.File]::WriteAllText($tempScriptFile, $newContent, $utf8NoBomEncoding)
...
# 确定要使用的输入文件
$inputFile = if ($needEmbedCode -and (Test-Path $tempScriptFile)) { $tempScriptFile } else { $mainScript }
    ...
    if (Test-Path $outputFile) {
        ...
        if ($fileInfo.Length -gt 1MB) {  # 确保文件不是空的或太小
            ...
            if ($needEmbedCode) {
                ...
                Write-Host "脚本已嵌入所有依赖文件的二进制内容" -ForegroundColor Green
